...
const Notes = ({state, dispatch}) => {
  const handleSubmit = async() => {
    ...
    title:state.title, note:state.note, recordId:state.selectedRecord.id, opened: "0", recordId: state.selectedRecord.id,
    ...
    const updateNote = async(data) => {
        const res = await axios.post(process.env.NEXT_PUBLIC_CLIMAX_UPDATE_SEAJOB_NOTES, {data}).then(() => {
     })}
  ...
  return (
    <div className='my-3' >
        ...
        {state.load?<Spinner size='sm' className='mx-2'/>:'Save'}</div>
        </div>}
        trigger="click">
        ...
        <Popover content={
        <div className='p-2 m-0' style={{border:'1px solid silver'}}>
        {state.load && <div style={{minWidth:400, maxHeight:100, textAlign:'center'}}>
        <Spinner size='sm' color='dark'/></div>}
        {!state.load && <>
